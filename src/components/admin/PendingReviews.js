// src/components/admin/PendingReviews.js - Ajout d'un onglet pour les avis rejetés
...
const PendingReviews = () => {
  ...
  useEffect(() => {
    ...
    fetchReviews();
  }, [currentUser, userDetails, sortOrder, activeTab]); // Ajouter activeTab aux dépendances
  ...
  const getTabTitle = () => {
    switch(activeTab) {
      case 'pending': return 'Avis en attente de validation';
    ...
    <ProfileLayout title={getTabTitle()}>
      {loading ? (
        ...
        <div>
          ...
          {/* Onglets pour filtrer par statut d'avis */}
          <div className="flex items-center space-x-4 mb-6 border-b border-gray-200">
            <button 
              onClick={() => setActiveTab('pending')}
              className={`px-4 py-2 ${activeTab === 'pending' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-600 hover:text-green-600'}`}
            >
              En attente
            </button>
            <button 
              onClick={() => setActiveTab('rejected')}
              className={`px-4 py-2 ${activeTab === 'rejected' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-600 hover:text-green-600'}`}
            >
              Rejetés
            </button>
            <button 
              onClick={() => setActiveTab('approved')}
              className={`px-4 py-2 ${activeTab === 'approved' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-600 hover:text-green-600'}`}
            >
              Approuvés
            </button>
          </div>
              ...
              {reviews.map((review) => {
                ...
                return (
                  <div key={review.id} className="bg-white rounded-lg shadow-sm overflow-hidden">
                    <div className="p-4">
                      {/* En-tête avec informations sur l'utilisateur et le produit */}
                      <div className="flex justify-between mb-4">
                        <div>
                          <h3 className="font-medium">
                            Avis de <span className="text-green-600">{review.users.display_name}</span> 
                            <span className="text-sm text-gray-500 ml-2">({review.users.email})</span>
                          </h3>
                        ...
                        <div className="flex items-center space-x-2">
                          {/* Boutons d'action différents selon l'onglet actif */}
                          {activeTab === 'pending' && (
                            <>
                      ...
                      {/* Contenu de l'avis (visible par défaut) */}
                      <div>
                        <div className="flex items-center mb-2">
                          {renderStars(review.average_rating)}
                          <span className="ml-2 font-medium">{review.average_rating?.toFixed(1) || '0.0'}/5</span>
                        </div>
                        
                        <p className="bg-gray-50 p-3 rounded-md text-gray-700">{review.comment}</p>
                      </div>
                      
                      {/* Détails de l'avis (visibles uniquement si développé) */}
                      {isExpanded && ratingsMap && (
                        <div className="mt-4 border-t border-gray-100 pt-4">
                          <h4 className="font-medium mb-2">Détails des notes</h4>
                          <div className="grid grid-cols-2 gap-2 mb-4">
                            {Object.entries(ratingsMap).map(([key, value]) => (
                              <div key={key} className="flex items-center">
                                <span className="text-sm text-gray-600 w-24">{value.display_name}:</span>
                                <div className="flex">
                                  {[1, 2, 3, 4, 5].map((star) => (
                                    <Star 
                                      key={star} 
                          ...
                          {(review.purchase_date || review.purchase_price || review.store_name) && (
                            <div className="mb-4">
                              <h4 className="font-medium mb-2">Informations d'achat</h4>
