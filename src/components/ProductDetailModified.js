// src/components/ProductDetailModified.js - Mise à jour avec intégration BDD
...
const ProductDetailModified = ({ product, interactionType = 'search', searchMetadata = {} }) => {
  ...
  // État pour le produit en base de données
  const [isSynced, setIsSynced] = useState(false);
  const [syncError, setSyncError] = useState(null);

  // Synchroniser le produit avec la base de données
  useEffect(() => {
    const syncProductWithDatabase = async () => {
      ...
      try {
        ...
        const { success, exists } = await findProductInDatabase(product.code);
        
        if (success) {
          if (!exists) {
            // Produit non trouvé, le sauvegarder
            ...
            if (saveResult.success) {
              console.log(`Produit ${product.code} sauvegardé avec succès`);
              setIsSynced(true);
            } else {
              console.error(`Erreur lors de la sauvegarde du produit ${product.code}:`, saveResult.error);
              setSyncError(saveResult.error);
            }
          } else {
            console.log(`Produit ${product.code} déjà en base`);
            setIsSynced(true);
          }
        } else {
          console.error("Erreur lors de la vérification en base:", syncError);
        }
      } catch (error) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="mt-6">
        ...
        {activeTab === 'avis' && <PageAvisEnhanced product={product} />}
      </div>
    </div>
  );
};

export default ProductDetailModified;