          ...
          {product && (
            <div className="mb-6 relative bg-white rounded-lg shadow-md" ref={productDetailRef}>
              <button 
                onClick={() => {
                  setProduct(null);
                  // Nettoyer l'URL en supprimant le code-barres
                  updateUrlParams(activeTab === 'barcode' ? 'barcode' : 'search', '', true);
                }}
                className="absolute top-2 right-3 bg-gray-200 text-gray-600 hover:bg-gray-300 p-2 rounded-lg flex items-center"
